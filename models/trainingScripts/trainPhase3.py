...
from models.phase3Autoencoder import ConfP3ConvAutoencoderFC
...
def train(args):
    ...
    model = ConfP3ConvAutoencoderFC().to(device)
    ...
    save_dir = f"checkpoints/Phase3/{args.dataset_name}"
    os.makedirs(save_dir, exist_ok=True)
    start_epoch = 0

    checkpoint_files = glob.glob(
        f"{save_dir}/P3autoencoder_{args.dataset_name}_epoch_*.pth"
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        torch.save(
            ...
            f"{save_dir}/P3autoencoder_{args.dataset_name}_epoch_{epoch+1}.pth",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cropImg",
        action="store_true",
        help="Include to crop the bottom half of images",
    )
    parser.add_argument(
        "--block_idx", type=int, default=0, help="Which chunk of the dataset to load"
    )
