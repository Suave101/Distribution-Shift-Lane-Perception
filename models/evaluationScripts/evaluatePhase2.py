...
# ==========================================
def evaluate_and_reconstruct(
    model,
    dataloader,
    dataset_name,
    model_name,
    device,
    output_dir="./reconstructions32",
    num_samples=20,
):
    ...
    with torch.no_grad():
        for images in tqdm(
            dataloader, desc=f"Evaluating {model_name} on {dataset_name}"
        ):
            ...
            if not saved_images:
                ...
                save_path = os.path.join(
                    output_dir, f"{model_name}_{dataset_name}_reconstructions.png"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, required=True, choices=["curvelanes", "culane"]
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    model = phase2Autoencoder.ConfP2ConvAutoencoderFC(latent_dim=32, configs=config).to(
        device
    )
